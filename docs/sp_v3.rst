3.0
===

Sponsored Products

Documentation: https://advertising.amazon.com/API/docs/en-us/sponsored-products/3-0/openapi/prod#/

This specification is available for download from the `Advertising API developer portal <https://dtrnk0o2zy01c.cloudfront.net/openapi/en-us/dest/SponsoredProducts_prod_3p.json>`_.


..  toctree::
    :caption: Table of Contents
    :maxdepth: 1

    sp/campaigns_v3
    sp/ad_groups_v3
    sp/bid_recommendations_v3
    sp/product_ads_v3
    sp/budget_rules
    sp/campaign_optimization_rules
    sp/campaigns_consolidated_recommendations
    sp/ranked_keywords_recommendations
    sp/keywords_v3
    sp/negative_keywords_v3
    sp/negative_product_targeting_v3
    sp/product_targeting
    sp/budget_recommendations
    sp/campaign_negative_targets
    sp/budget_rules_recommendations
    sp/product_recommendations
    sp/campaign_budget_usage
    sp/campaign_negative_keywords_v3